...
class CrdCommand(command.OpenStackCommand):
    api = 'crd'
    log = logging.getLogger(__name__ + '.CrdCommand')
    values_specs = []
    json_indent = None

    def get_client(self):
        return self.app.client_manager.crd

    def get_parser(self, prog_name):
        parser = super(CrdCommand, self).get_parser(prog_name)
        parser.add_argument(
            '--request-format',
...
class CreateCommand(CrdCommand, show.ShowOne):
    ...
    def get_data(self, parsed_args):
        ...
        crd_client = self.get_client()
        crd_client.format = parsed_args.request_format
        _extra_values = parse_args_to_dict(self.values_specs)
        _merge_args(self, parsed_args, _extra_values,
                    self.values_specs)
        body = self.args2body(parsed_args)
        body[self.resource].update(_extra_values)
        obj_creator = getattr(crd_client,
...
class UpdateCommand(CrdCommand):
    ...
    def run(self, parsed_args):
        ...
        _id = find_resourceid_by_name_or_id(crd_client,
                                            self.resource,
                                            parsed_args.id)
        obj_updator = getattr(crd_client,
...
class DeleteCommand(CrdCommand):
    ...
    def run(self, parsed_args):
        ...
        crd_client = self.get_client()
        crd_client.format = parsed_args.request_format
        obj_deleter = getattr(crd_client,
                              "delete_%s" % self.resource)
        if self.allow_names:
            _id = find_resourceid_by_name_or_id(crd_client, self.resource,
                                                parsed_args.id)
        else:
            _id = parsed_args.id
        params = {'body':params}    
...
class ListCommand(CrdCommand, lister.Lister):
    ...
    def call_server(self, crd_client, search_opts, parsed_args):
        obj_lister = getattr(crd_client,
    ...
    def retrieve_list(self, parsed_args):
        ...
        data = self.call_server(crd_client, search_opts, parsed_args)
    ...
    def extend_list(self, data, parsed_args):
        ...
        the crd server. For example, you can add subnet cidr information
        to a list network.
        """
        pass
...
class ShowCommand(CrdCommand, show.ShowOne):
    ...
    def get_data(self, parsed_args):
        ...
        if self.allow_names:
            ...
            _id = parsed_args.id
        params = {'body':params}
        obj_shower = getattr(crd_client, "show_%s" % self.resource)
