...
* Exceptions from client library:
  This type of exceptions should inherit CrdClientException.
* Exceptions from CLI code:
  This type of exceptions should inherit CrdCLIError.
"""


class CrdException(Exception):
    """Base Crd Exception.
...
class CrdClientException(CrdException):
    """Base exception which exceptions from Crd are mapped into.
    ...
    def __init__(self, message=None, **kwargs):
        ...
        super(CrdClientException, self).__init__(message, **kwargs)


# Base exceptions from Crd

class BadRequest(CrdClientException):
    status_code = 400


class Unauthorized(CrdClientException):
    status_code = 401
    message = _("Unauthorized: bad credentials.")


class Forbidden(CrdClientException):
    status_code = 403
    message = _("Forbidden: your credentials don't give you access to this "
                "resource.")


class NotFound(CrdClientException):
    status_code = 404


class Conflict(CrdClientException):
    status_code = 409


class InternalServerError(CrdClientException):
    status_code = 500


class ServiceUnavailable(CrdClientException):
    status_code = 503
...
# Exceptions mapped to Crd server exceptions
# These are defined if a user of client library needs specific exception.
# Exception name should be <Crd Exception Name> + 'Client'
...
# TODO(amotoki): It is unused in Crd, but it is referred to
...
class NoAuthURLProvided(Unauthorized):
    message = _("auth_url was not provided to the Crd client")


class EndpointNotFound(CrdClientException):
    message = _("Could not find Service or Region in Service Catalog.")


class EndpointTypeNotFound(CrdClientException):
    message = _("Could not find endpoint type %(type_)s in Service Catalog.")


class AmbiguousEndpoints(CrdClientException):
    message = _("Found more than one matching endpoint in Service Catalog: "
                "%(matching_endpoints)")


class RequestURITooLong(CrdClientException):
...
class ConnectionFailed(CrdClientException):
    message = _("Connection to neutron failed: %(reason)s")


class SslCertificateValidationError(CrdClientException):
    message = _("SSL certificate validation has failed: %(reason)s")


class MalformedResponseBody(CrdClientException):
    message = _("Malformed response body: %(reason)s")


class InvalidContentType(CrdClientException):
    message = _("Invalid content type %(content_type)s.")


# Command line exceptions

class CrdCLIError(CrdException):
    """Exception raised when command line parsing fails."""
    pass


class CommandError(CrdCLIError):
    pass


class UnsupportedVersion(CrdCLIError):
...
class CrdClientNoUniqueMatch(CrdCLIError):
