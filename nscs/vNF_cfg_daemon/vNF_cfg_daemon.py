...
class VMConfigurationDaemon(object):
	...
	#    (output,error) = p.communicate()
		...
		net_id = port['net_id']
		if dev == 'b9131':
		    ports.update({'b9131':{'port_id': port_id, 'net_id': net_id, 'ipaddr': ipaddr, 'macaddr': None}})
		    cmd = ""
		    #cmd = "ifconfig eth0 " +str(ipaddr)+ "/24 up"
		elif dev == 'epc':
		    macaddr = port['macaddr']
		    ports.update({'epc':{'port_id': port_id, 'net_id': net_id, 'ipaddr': ipaddr, 'macaddr': macaddr}})
		    cmd = "ifconfig eth0 " +str(ipaddr)+ "/24 up"
