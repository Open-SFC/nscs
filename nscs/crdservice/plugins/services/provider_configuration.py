...
for cnf in moduleconfs:
	if re.search(r'(\.conf)$', cnf):
		...
		service_plugin = ''
		try:
	                service_plugin = str(modconf.get("service_providers","service_provider"))
			if not service_plugin in serviceproviders:
				serviceproviders.append(service_plugin)
		except:
			LOG.debug(_("No Service Provider for %s\n"), str(cnf))
LOG.debug(_("No Service Provider for dddddddddddddddd %s\n"), str(serviceproviders))
...
def parse_service_provider_opt():
    ...
    svc_providers_opt = serviceproviders
    res = []
