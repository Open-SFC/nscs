...
class DnsmasqFilter(CommandFilter):
    ...
    def is_dnsmasq_env_vars(self, argv):
        if (argv[0].startswith("CRD_RELAY_SOCKET_PATH=") and
            argv[1].startswith("CRD_NETWORK_ID=")):
            return True
        return False
    ...
    def get_environment(self, userargs):
        env = os.environ.copy()
        env['CRD_RELAY_SOCKET_PATH'] = userargs[0].split('=')[-1]
        env['CRD_NETWORK_ID'] = userargs[1].split('=')[-1]
        return env
