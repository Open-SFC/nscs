...
from nscs.crdserver.api import api_common
from nscs.crdserver.api.v2 import attributes
from nscs.crdserver.api.v2 import resource as wsgi_resource
from nscs.crdserver.common import exceptions
from nscs.crdserver.openstack.common import log as logging
from nscs.crdserver.openstack.common.notifier import api as notifier_api
from nscs.crdserver import policy
#from crd import quota
...
class Controller(object):
    ...
    def __init__(self, plugin, collection, resource, attr_info,
        ...
        if parent and (type(parent) is dict):
            self._parent_ids = None
            self._parent_id_name = '%s_id' % parent['member_name']
            parent_part = '_%s' % parent['member_name']
        elif parent and (type(parent) is list):
            self._parent_ids = []
            self._parent_id_name = None
            parent_part = ''
            for p in parent:
                self._parent_ids.append('%s_id' % p['member_name'])
                parent_part = '_%s' % p['member_name'] + parent_part
        else:
            self._parent_id_name = None
            self._parent_ids = None
            parent_part = ''
    ...
    def __getattr__(self, name):
        LOG.debug(_("AAAAAAAAAAAAAAAAAAAAAAAAA %s"),str(name))
        LOG.debug(_("AAAAAAAAAAAAAAAAAAAAAAAAA %s"),str(self._member_actions))
    ...
    def index(self, request, **kwargs):
        ...
        return self._items(request, True, parent_ids)
