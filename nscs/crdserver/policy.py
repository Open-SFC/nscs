...
"""
Policy engine for crd.  Largely copied from nova.
"""
import itertools
import re
...
cfg.CONF.import_opt('policy_file', 'nscs.crdserver.common.config')
...
class OwnerCheck(policy.Check):
    ...
    def __call__(self, target, creds):
        if self.target_field not in target:
            ...
            context = importutils.import_module('nscs.crdserver.context')
            try:
...
def enforce(context, action, target, plugin=None):
    ...
    :raises crd.exceptions.PolicyNotAuthorized: if verification fails.
    """
